	...
	function logTokenUrlWithUser(req) {
		...
		var userStringLength = 15;
		if (req.session && typeof req.user === 'object') {
			sections.push(pad('u:' + req.user.id + ' g:' + req.user.group_id , userStringLength, ' ').gray)
		}
		else{
			sections.push(pad('no user', userStringLength, ' ').gray)
		}
	...
	exports.addTokens = function (morgan){
		...
		morgan.token('customUa', function(req) {
			if (req.headers && req.url && req.url.match('session/init')) {
