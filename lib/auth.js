(function () {
    ...
    function deserializeUserMysql(string, done) {
		...
		var q = 'select u.* , ug.*, GROUP_CONCAT( role ) roles from users u  join user_roles r on(u.id=r.user_id) join user_groups ug on(ug.id = u.group_id) where u.id = ? group by user_id'
		var p = [string]
