...
(function () {
	...
	exports.connectToResources = function(resources) {
		...
		resources.forEach(function(r){
			...
			t.once(r + '-connected', function(){
                ...
                debug(JSON.stringify(connectedHash))

				delete connectedHash[r]
				if(Object.keys(connectedHash).length == 0){
					clearTimeout(timer)
	...
	exports.startOnResourcesConnected = function(_port){
		var t = this;
        ...
        try{
			var pjson = require(module.parent.parent.filename + '/../package.json');
		...
		this.once('resources-connected', function(){
		    t.start(_port || 0, function(err, port) {
                ...
                console.info((appName || process.title) + " listening on port %d (pid: " + process.pid + "), using service-scaff@" + scaffVersion, port)
		    });
		})
	}
})()
